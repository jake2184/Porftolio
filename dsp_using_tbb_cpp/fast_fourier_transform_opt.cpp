...
namespace jr2812
{
class fast_fourier_transform_opt : public fourier_transform
{
protected:
	...
	{
		// Create default values dependant on n and numCores
		// If statements/maths/extra function add overhead, but allow flexible default
		// Dependant both on n and hardware

		unsigned chunkSize;
		if(HPCE_FFT_LOOP_K == 0){
			// Aim for numCore threads, but have 2*numCore threads for flexibility
		...
		if(HPCE_FFT_RECURSION_K == 0){
			// Seems to make little difference as long as you have >numCores threads
			// eg recursionLimit <= n / numCores
			recursionLimit = n / (numCores + numCores);
		} else{
			recursionLimit = n / HPCE_FFT_RECURSION_K;
		}
	...
	) const 
	{
		...
		if (n == 1){
			...
			// Optional parallel task recursion and chunked parallel recombination
			size_t m = n/2;

			//Divide 
			if(n <= recursionLimit){
				// Normal recursion
				my_forwards_impl(m,wn*wn,pIn,2*sIn,pOut,sOut, chunkSize, recursionLimit);
				my_forwards_impl(m,wn*wn,pIn+sIn,2*sIn,pOut+sOut*m,sOut, chunkSize, recursionLimit);
			}else{
				// Parallel recursion (to a degree)
				tbb::task_group group;
				group.run( [=]() {my_forwards_impl(m,wn*wn,pIn,2*sIn,pOut,sOut, chunkSize, recursionLimit);} );
				//group.run( [=]() {my_forwards_impl(m,wn*wn,pIn+sIn,2*sIn,pOut+sOut*m,sOut, chunkSize, recursionLimit);} );
...
public:
	...
	{ return "hpce.jr2812.fast_fourier_transform_opt"; }
	
	virtual bool is_quadratic() const
	{ return false; }

	fast_fourier_transform_opt(){
		char *envVar = getenv("HPCE_FFT_RECURSION_K");
		if(envVar != NULL){
			HPCE_FFT_RECURSION_K = atoi(envVar);
		} else {
			HPCE_FFT_RECURSION_K = 0;
		}
