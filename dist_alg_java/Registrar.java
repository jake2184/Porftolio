...
public class Registrar {
	...
	public final int pid = 0; /* P0 is reserved for this server */
	...
	/* A lock as a barrier to synchronize all clients */
	private Object lock;
	
	public RoutingOracle oracle;
	
	private int [][] weights; /* Delay per link */
	...
	public Registrar(int n, String filename) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < i; j++) {
				if (oracle.areNeighbours(i, j)) {
					if (Utils.delay == Utils.Delay.RANDOM) {
						weights[i][j] = weights[j][i] = Utils.DELAY + random.nextInt(Utils.DELAY);
					} else { 
						/* Message delay is uniform */
						weights[i][j] = weights[j][i] = Utils.DELAY;
					}
					/*
					 * Debug
					 *
					 * Utils.out(String.format("<%2d, %2d> %4d (%4d)", 
					 * 		i + 1, j + 1, weights[i][j], weights[j][i]));
					 */
				}
			}
		}
	...
	public int getDelay(int src, int dst) {
	...
	public boolean areRegistered () { /* The condition for synchronization */
